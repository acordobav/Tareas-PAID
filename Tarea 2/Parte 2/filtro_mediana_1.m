...
%Matriz donde se guardan los marco del video filtrados
Y = uint8(zeros(m,n,3,frames));

%Leer el video
for k = 1:frames
   Z = readFrame(V); %Leer cada uno de los marcos del video
   Y(:,:,1,k) = mediana(Z(:,:,1)); %Informacion Canal Rojo
   Y(:,:,2,k) = mediana(Z(:,:,2)); %Informacion Canal Verde
   Y(:,:,3,k) = mediana(Z(:,:,3)); %Informacion Canal Azul
endfor

%Crear el nuevo video
video = VideoWriter('video_sin_ruido_1.mp4'); %Crear el video vacio
for i = 1:frames
